...
def tratar_clientes_desde(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
...
def tratar_contratacoes_12m(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
    df = padronizar_colunas(df)
...
def tratar_dados_clientes(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
    df = padronizar_colunas(df)
...
def tratar_nps_relacional(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
    df = padronizar_colunas(df)
...
def tratar_nps_implantacao(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
    df.columns = df.columns.str.replace('Cód. Cliente','CD_CLIENTE').str.replace('Data da Resposta','DATA_RESPOSTA')
    if 'DATA_RESPOSTA' in df.columns:
        df['DATA_RESPOSTA'] = pd.to_datetime(df['DATA_RESPOSTA'], errors='coerce')
...
def tratar_nps_onboarding(blob):
    df = pd.read_csv(blob, encoding='latin1', sep=';', on_bad_lines='skip')
    renomear = {
...
def tratar_telemetria(blob):
    ...
    df = df.drop_duplicates()
    print(f"✅ Arquivo telemetria tratado")
    return df
